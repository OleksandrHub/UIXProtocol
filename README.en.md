...
- `getClientIp(req)` ([server-proxy.ts](scripts/server-proxy.ts)) — returns the real client IP: the first entry of `X-Forwarded-For` (set by a trusted nginx/reverse-proxy) if present, otherwise `req.socket.remoteAddress`. Strips the `::ffff:` prefix for IPv4-mapped addresses.
- `performProxy(req, res, targetRaw, pathOnly, opts)` ([server-proxy.ts](scripts/server-proxy.ts)) — performs the upstream http(s) request, rewrites `Set-Cookie` (drops `Domain`, `Secure`, forces `SameSite=Lax`), strips `X-Frame-Options` / `Content-Security-Policy` / `Strict-Transport-Security` / `Feature-Policy`, injects its own `Permissions-Policy` from `iframePermissions`. Explicitly overwrites `X-Forwarded-For` and `X-Real-IP` with `getClientIp(req)` (so the upstream reliably sees the user's real IP regardless of the nginx config) and removes `Via` / `Forwarded` (RFC proxy markers) from the outgoing request. For `text/html` and `application/javascript` it buffers the body and runs `rewriteUrls`. Options (`ProxyOpts` from [types.ts](scripts/types.ts)):
...
- **Client IP** is forwarded to the upstream via standard `X-Forwarded-For` / `X-Real-IP` (value sourced from `getClientIp(req)`). `Via` and `Forwarded` (RFC proxy markers) are stripped from the outgoing request. For high-security upstreams (Cloudflare Bot Management, DataDome) the proxy fact is still detectable via the datacenter ASN of the server IP and TLS fingerprint — that layer is out of scope here.
