...
- `getClientIp(req)` ([server-proxy.ts](scripts/server-proxy.ts)) — повертає реальну IP клієнта: спочатку перший запис із `X-Forwarded-For` (виставленого довіреним nginx/reverse-proxy), інакше `req.socket.remoteAddress`. Зачищає префікс `::ffff:` для IPv4-mapped адрес.
- `performProxy(req, res, targetRaw, pathOnly, opts)` ([server-proxy.ts](scripts/server-proxy.ts)) — виконує http(s)-запит до таргета, переписує `Set-Cookie` (зрізає `Domain`, `Secure`, форсить `SameSite=Lax`), знімає `X-Frame-Options` / `Content-Security-Policy` / `Strict-Transport-Security` / `Feature-Policy`, виставляє `Permissions-Policy` зі списку `iframePermissions`. Явно перезаписує `X-Forwarded-For` і `X-Real-IP` на результат `getClientIp(req)` (щоб таргет надійно бачив реальну IP користувача, незалежно від конфігу nginx), і вирізає `Via` / `Forwarded` (RFC-маркери «тут проксі») з вихідного запиту. Для `text/html` та `application/javascript` — буферизує тіло і викликає `rewriteUrls`. Опції (`ProxyOpts` із [types.ts](scripts/types.ts)):
...
- **IP клієнта** надійно передається таргету через стандартні `X-Forwarded-For` / `X-Real-IP` (значення береться з `getClientIp(req)`). Заголовки `Via` і `Forwarded` (RFC-маркери проксі) вирізаються з вихідного запиту. Для високозахищених сайтів (Cloudflare Bot Management, DataDome) сам факт проксі однаково детектиться по datacenter-ASN серверного IP і TLS-fingerprint — цей рівень тут не лікується.
