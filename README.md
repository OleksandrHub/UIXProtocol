...
Той самий `installShortcuts()` слухає `wheel` (passive: false, capture) і також дублюється в iframe. Cooldown — **700 мс** між діями, щоб довге прокручування не спамило запитами.

| Жест | Дія |
| --- | --- |
| Колесо вгору (`deltaY < 0`) | `triggerGemini()` — викликати Gemini |
| Колесо вниз (`deltaY > 0`) | `toggleResult()` — toggle відповіді |

Подія `wheel` блокує дефолтний скрол (`preventDefault`) — так і задумано: керування заточене під швидкий "скін → відповідь" сценарій.
